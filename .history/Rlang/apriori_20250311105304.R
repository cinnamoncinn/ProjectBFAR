...
# Load dataset
data <- read.csv("dataset/cleaned_data.csv", stringsAsFactors = FALSE)
...
# Ensure columns are logical before conversion
transactions[] <- lapply(transactions, function(x) as.logical(x))

# Convert TRUE/FALSE to "Yes"/"No"
transactions[] <- lapply(transactions, function(x) ifelse(x, "Yes", "No"))

# Convert data frame to transactions
transactions <- as(as.matrix(transactions), "transactions")

# Apply Apriori algorithm with optimized parameters
rules <- apriori(transactions, parameter = list(supp = 0.03, conf = 0.6, maxlen = 7))

# Visualize the rules
plot(rules, method="graph", engine="igraph", control=list(max=200))